...
export default function AdminPage() {
  ...
  async function handleCatalogSync() {
    setBusy(true);
    try {
      const result = await adminFetch<{ synced: number }>("/api/v1/admin/extensions/catalog/sync", savedToken, { method: "POST" });
      const loaded = await loadSources(savedToken);
      await loadSourceStatuses(loaded, savedToken);
      setMessage(`Catalog synced. ${result.synced} extensions tersinkron.`);
      pushToast({ tone: "success", title: "Catalog synced", message: `${result.synced} extensions loaded` });
    } catch (error) {
      const errorMessage = error instanceof Error ? error.message : "Gagal sync catalog.";
      setMessage(errorMessage);
      pushToast({ tone: "error", title: "Catalog sync failed", message: errorMessage });
    } finally {
      setBusy(false);
    }
  }
