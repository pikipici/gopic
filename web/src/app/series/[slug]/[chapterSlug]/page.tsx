...
export default async function ReaderPage({ params }: PageProps) {
  ...
  const chapterIndex = seriesDetail?.chapters.findIndex((item) => item.slug === chapter.slug) ?? -1;
  const previous = chapterIndex >= 0 ? seriesDetail?.chapters[chapterIndex + 1] : undefined;
  const next = chapterIndex >= 0 ? seriesDetail?.chapters[chapterIndex - 1] : undefined;
